...
class DataVectorTask : public thread::Task {
public:
    enum class Type { dataToVector, dataFromVector };
    DataVectorTask(BMP* self, Type type, int start, int end)
        : self_(self), type_(type), start_(start), end_(end) {}

    void run() override {
        if (type_ == Type::dataToVector) {
            self_->dataToVectorPartial(start_, end_);
        } else {
            self_->dataFromVectorPartial(start_, end_);
        }
    }
...
void BMP::dataToVector() {
    ...
    for (unsigned i = 0; i < pool.count(); ++i) {
        int start = i * height() / pool.count();
        int end = (i + 1) * height() / pool.count();
        pool.add(new DataVectorTask(this, DataVectorTask::Type::dataToVector, start, end));
    }
    pool.waitForTasks();
}
...
void BMP::dataFromVector() {
    for (unsigned i = 0; i < pool.count(); ++i) {
        int start = i * height() / pool.count();
        int end = (i + 1) * height() / pool.count();
        pool.add(new DataVectorTask(this, DataVectorTask::Type::dataFromVector, start, end));
    }
    pool.waitForTasks();
}

void BMP::dataToVectorPartial(int start, int end) {
...
void BMP::dataFromVectorPartial(int start, int end) {
    for (int row = start; row < end; ++row) {
        for (int col = 0; col < width(); ++col) {

...
#define MSG_REPORT_FOR_PEAK         "max"
...
struct ResourceUsageEntry {
    int m, d, hoursUsage[HOUR_COUNT];
};

struct BillEntry {
    int m, water, gas, electricity;
};
...
public:
    ...
    int mean(int m);
    int diff(int m);
    int water(int m, int p);
    int gas(int m, int p);
    int electricity(int m, int p);
    int totalUsage(int m, int hour);
    std::vector<int> maxUsage(int m);
...
public:
    ...
    std::vector<int> calcParam(int m);
